...
Ind_NYC_2019_cleaned<-data.frame("neighbourhood_group"= Ind_NYC$Geographic.Identifier,
                                 "Indicator"=Ind_NYC$Indicator,"Incidents"=Ind_NYC$FY2019)
...
The model is plotted using three different regression lines per room type. There is a tendency for all room types that the price is lower if the place is further from Times Square. The interactions are not significant.
...
map<-leaflet(NYC) %>%
  ...
  addCircleMarkers(lng = NYC$longitude, 
                   ...
                   popup = paste0("Price per night: ", as.character(NYC$price), " $",
                                  ...
                                  as.character(round(NYC$dist.timessquare/1000, digits=1)),
                                  " km"),
